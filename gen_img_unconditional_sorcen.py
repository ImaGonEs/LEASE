...
parser = argparse.ArgumentParser('Sorcen generation', add_help=False)
...
parser.add_argument('--ckpt', type=str,
                    help='checkpoint')
parser.add_argument('--model', default='inference_sorcen_vit_base_patch16_single', type=str,
                    help='model')
parser.add_argument('--output_dir', default='output_dir/fid/gen/sorcen-vitb', type=str,
                    help='name')
...
model = models_lease_tk_1600.__dict__[args.model](norm_pix_loss=False,
...
filtered_state_dict = {k: v for k, v in state_dict.items() if not (k.startswith('classifier.2.weight') or k.startswith('classifier.2.bias'))}


missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)

# Print missing and unexpected keys (incompatible layers)
if missing_keys:
    print(f"Missing keys (layers not found in checkpoint):\n {missing_keys}")
if unexpected_keys:
    print(f"Unexpected keys (layers in checkpoint but not in model):\n {unexpected_keys}")
...
for i in tqdm(range(num_steps)):
    ...
    for b_id in range(args.batch_size):
        ...
        # Save the image
        save_path = os.path.join(save_folder, '{}.png'.format(str(i*args.batch_size+b_id).zfill(5)))
